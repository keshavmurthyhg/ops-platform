...
# ─────────────────────────────────────────────────────────────────────────────
#  SMTP config status — returns a dict describing current config health
# ─────────────────────────────────────────────────────────────────────────────

def get_email_status() -> dict:
    """Return current SMTP configuration status without sending anything."""
    password = SMTP_PASSWORD or os.environ.get("EMAIL_PASSWORD", "")
    status = {
        "smtp_host"      : SMTP_HOST,
        "smtp_port"      : SMTP_PORT,
        ...
        "password_source": ("env:EMAIL_PASSWORD" if os.environ.get("EMAIL_PASSWORD") else
                            ("hardcoded" if SMTP_PASSWORD else "NOT SET")),
        "ready"          : bool(password),
    }
    if not password:
        status["error"] = (
            "EMAIL_PASSWORD is not set.\n"
            "Fix: in your terminal BEFORE starting Flask, run:\n"
            "  Windows:  set EMAIL_PASSWORD=your_password\n"
            "  Linux:    export EMAIL_PASSWORD=your_password\n"
            "Then restart Flask."
        )
    return status
